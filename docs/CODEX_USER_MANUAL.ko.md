...
대시보드는 coordinator와 같은 프로세스에서 실행되며, 기본적으로 다른 로컬 컴퓨터에서도 접속할 수 있도록 `0.0.0.0`에 bind된다. 호스트 PC의 LAN IP와 port를 사용해 `http://<호스트-LAN-IP>:9400`으로 접속한다. 로컬 PC에서만 접근하도록 제한하려면 `--dashboard-host 127.0.0.1`을 사용한다. Windows Defender 방화벽이 물으면 Python 또는 TCP port 9400의 Private network 접근을 허용해야 한다. 문제 상태는 약 2.5초마다, 각 solver Docker의 CPU·메모리·PID·network 통계는 기본 1초마다 화면에 반영된다. 쓰기 요청은 브라우저가 같은 origin에서 받은 세션별 token을 요구한다.
